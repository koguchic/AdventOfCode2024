...
def calculate_antinode_pos(tower_1, tower_2):
    ...
    # Do it for negative ray
    starting_point = tower_1.copy()
    while True:
        pos = [x - y for x, y in zip(starting_point, delta)]
    ...
    # Do it for positive ray
    starting_point = tower_2.copy()
    while True:
        pos = [x + y for x, y in zip(starting_point, delta)]
        if 0 <= pos[0] < len(grid) and 0 <= pos[1] < len(grid[0]):
            positions.append(pos)
...
tower_frequencies = Counter(flattened)
del tower_frequencies[EMPTY]
...
for tower_freq in tower_frequencies:
    ...
    for i in range(len(tower_locations)):
        for j in range(i+1, len(tower_locations)):
            ...
            positions = calculate_antinode_pos(tower_1, tower_2)
            for pos in positions:
                if 0 <= pos[0] < len(grid) and 0 <= pos[1] < len(grid[0]):
                    answer[pos[0]][pos[1]] = ANTINODE


# All towers themselves become antinodes as long as there's 2 e.g. forms a line
for i in range(len(grid)):
    for j in range(len(grid[0])):
        c = grid[i][j]
        if c in tower_frequencies and tower_frequencies[c] > 1:
            answer[i][j] = ANTINODE
...
print(f'Puzzle answer: {num_antinode}') # 1064
